...
public class KimiProvider : ILLMProvider
{
    ...
    public KimiProvider(ToolRegistry tools, HttpClient httpClient, INotificationService? notification = null)
    {
        _tools = tools;
    ...
    public virtual string ProviderType => "kimi";
    protected virtual string DefaultEndpoint => "https://api.moonshot.cn/v1";
    protected virtual bool AddReasoningEffort => false;
        ...
        try
        {
            ...
            for (var iteration = 0; iteration < 12; iteration++)
            {
                ...
                if (!message.TryGetProperty("tool_calls", out var toolCallsNode) || toolCallsNode.ValueKind != JsonValueKind.Array || toolCallsNode.GetArrayLength() == 0)
                    return new() { Success = true, Content = content, InputTokens = inputTokens, OutputTokens = outputTokens, ToolCalls = executedCalls };
    ...
    private static object BuildUserContent(string userMessage, MessageAttachment? attachment)
    {
        ...
        return new List<object>
        {
            ...
            new { type = "image_url", image_url = new { url = $"data:{attachment.MimeType};base64,{attachment.Base64Data}" } }
        };
    }
}